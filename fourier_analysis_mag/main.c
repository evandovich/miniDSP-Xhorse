...
//                  : Notable computation: Polar Notation - Magnitude and Phase
//
// Revision         : V2.0
// Additional Comments:
//////////////////////////////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////////////////////////
// Function: dft_signal_calc_
// Description:
//              Computes the Fourier Transformation of the signal
// Function: rect_to_polar_
// Description:
//              This function obtains a data signal and computes the Polar notation information
//              i.e. Outputs the Magnitude and Phase of signal
//////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    void rect_to_polar_(double *sig_src_real_arr,
...
int main()
{
    ...
    double OUTPUT_MAGN[SIG_LENGTH/2];
    double OUTPUT_PHASE[SIG_LENGTH/2];

    dft_signal_calc(&INPUT_SIG_ARR[0], &OUTPUT_REAL[0], &OUTPUT_IMX[0], SIG_LENGTH );
    rect_to_polar_(&OUTPUT_REAL[0], &OUTPUT_IMX[0], &OUTPUT_MAGN[0], &OUTPUT_PHASE[0], SIG_LENGTH);

     fptr2 = fopen("output_mag.dat", "w");
     fptr3 = fopen("output_phase.dat", "w");
...
void rect_to_polar_(double *sig_src_real_arr, double *sig_src_imx_arr, double *sig_out_mag_arr,double *sig_out_phase_arr, int sig_Length)
{
     double PI = 3.14159265358979f;
     int k;
     for (k=0;k<sig_Length/2;k++)
     {
         sig_out_mag_arr[k] = sqrt(powf(sig_src_real_arr[k],2)+ powf(sig_src_imx_arr[k],2));
         //Conditions
         if (sig_src_real_arr[k] == 0)
         {
             sig_src_real_arr[k] = pow(10,-20);
             sig_out_phase_arr[k] = atan(sig_src_imx_arr[k]/sig_src_real_arr[k]);
         }
         if((sig_src_real_arr[k]<0)&&(sig_src_imx_arr[k]<0))
         {
